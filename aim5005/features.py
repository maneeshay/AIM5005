...
class MinMaxScaler:
    ...
    def _check_is_array(self, x: np.ndarray) -> np.ndarray:
        """
        Try to convert x to a np.ndarray if it's a not a np.ndarray and return. If it can't be cast raise an error
        """
    ...
    def transform(self, x: np.ndarray) -> list:
        ...
        # Fix the bug: Properly apply the scaling formula
